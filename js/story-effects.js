...
((story.config = {
  ...
  (story.load = {
    init: function (callback1) {
      try {
        var cachedStoryData = localStorage.getItem("storyData");
        story.config.storyData = cachedStoryData
          ? JSON.parse(cachedStoryData)
          : null;
      } catch (error) {
        story.config.storyData = null;
        try {
          localStorage.removeItem("storyData");
        } catch (storageError) {
          // Continue with the network fallback when storage is unavailable.
        }
      }
      ...
      xobj.onreadystatechange = function () {
        if (xobj.readyState == 4) {
          if (xobj.status < 200 || xobj.status >= 300) {
            story.load.showError("Unable to load the story.");
            return;
          }
          ...
          try {
            ...
            try {
              localStorage.setItem(
                "storyData",
                JSON.stringify(story.config.storyData),
              );
            } catch (storageError) {
              // The story can still run when browser storage is unavailable.
            }
            story.load.loadPage();
          } catch (error) {
            story.load.showError("The story data is unavailable.");
          }
        }
      };
    ...
    //manifest of each page's background images, generated by scripts/generate-preload-manifest.js
    imageManifest: null,
    loadImageManifest: function () {
      if (story.load.imageManifest) {
        return Promise.resolve(story.load.imageManifest);
      }
      return fetch("./assets/preload-manifest.json")
        ...
        .then(function (manifest) {
          story.load.imageManifest = manifest;
          return manifest;
        });
    },

    //resolves once every url has loaded (or failed) so a single broken asset can't hang the story
    preloadImages: function (urls) {
      ...
      var timeout = new Promise(function (resolve) {
        story.runtime.setTimeout(resolve, 5000);
      });

      return Promise.race([Promise.all(loaders), timeout]);
    },

    //warms the browser cache for the next page so its images are ready before the reader taps "next"
    prefetchNextPageImages: function (manifest) {
      var nextPageNo = Number(story.config.currentPage) + 1;
      if (!isFinite(nextPageNo) || nextPageNo > 25) {
        return;
      }
      story.load.preloadImages(manifest["page" + nextPageNo]);
    },

    loadPage: function () {
      var currentPage = story.config.currentPage;
      var storyData = story.config.storyData;
      var pageEntry =
        storyData && Array.isArray(storyData.pages)
          ? storyData.pages[currentPage]
          : null;

      if (!pageEntry || !Array.isArray(pageEntry.page)) {
        story.load.showError("The story page is unavailable.");
        return;
      }

      var pageData = pageEntry.page;
      ...
      for (var i = 1; i <= totalSentencesOnPage; i++) {
        ...
        sentenceElement.className = "sentence-" + i + " js-sentence-" + i;
        sentenceElement.textContent = sentence;
        sentenceElement.tabIndex = 0;
        main[0].appendChild(sentenceElement);
      }

      main[0].setAttribute("aria-live", "polite");
      main[0].setAttribute("aria-label", "Story text");

      if (story.effects.reducedMotion()) {
        story.config.readToMe = false;
      }
      ...
      if (story.interactions.init()) {
        var pageKey = "page" + story.config.currentPage;

        story.load.loadImageManifest().then(function (manifest) {
          story.load.preloadImages(manifest[pageKey]).then(function () {
            ...
            story.load.prefetchNextPageImages(manifest);
          });
        });
      }
    },
  ...
  (story.interactions = {
    navigation: {
      loadNextSentence: function (e, button) {
        ...
        document
          .querySelector(".js-sentence-" + currentSentence)
          .classList.add("cssFadeOut");
        document
          .querySelector(".js-sentence-" + nextSentence)
          .classList.add("cssFadeIn");
    ...
    readToMe: function () {
      ...
      var currentSentence = story.config.currentSentence;
      var currentSentenceData =
        story.config.storyData.pages[currentPage].page[currentSentence - 1];

      var currentWord = 1;

      story.interactions.playSentence();

      story.config.sentenceAnimation = new TimelineMax({
        delay: story.config.sentenceAnimationDelay,
      });
      ...
      for (var i = 0; i < animationCount; i++) {
        ...
        var wordAnimation = TweenMax.to(word, timings[i], {
        ...
        var wordAnimation = TweenMax.to(word, 0.1, {
          css: { className: "-=highlight" },
          ease: Power2.easeOut,
        });
    ...
    playSentence: function () {
      ...
      story.effects.playAudio(
        sound_dir +
          "/page" +
          story.config.currentPage +
          "/" +
          story.config.currentSentence +
          ".mp3",
        "narration",
    ...
    init: function () {
      ...
      $(".button-back").attr("aria-label", "Previous page");
      $(".button-next").attr("aria-label", "Next sentence");
      ...
      $("main p").on("click.storyRuntime keydown.storyRuntime", function (e) {
      ...
      $(".menu-button").on(
        "click.storyRuntime keydown.storyRuntime",
        function (e) {
          if (e.type === "keydown" && e.key !== "Enter" && e.key !== " ") {
            return;
          }
          ...
          var menuIsOpen = menuContainer
            .toggleClass("slide-in")
            .hasClass("slide-in");
          $(this).attr("aria-expanded", String(menuIsOpen));
          $(".menu-container")
            .find(".fa")
      ...
      if (typeof page !== "function") {
        throw new Error(
          "No story page registered for " + story.config.currentPage,
        );
      }
      page();

      story.interactions.makeHotspotsAccessible();

      return true;
    },
    //keyboard access for the click-only story hotspots bound in story-main.js
    hotspotSelectors: [
      ".js-sheep1-container",
      ".js-sheep3-container",
      ".js-sheep-container",
      ".sheep1-trigger",
      ".js-rocksheep-container",
      ".js-amplifier-container",
      ".js-vinyl-cover",
      ".js-moon",
      ".js-sleep",
      ".js-rocksheep",
      ".js-pirate-sheep",
      ".swim-sheep-container",
      ".croc-sheep-container",
      ".squirrel-sheep-container",
      ".hotdog-sheep-container",
      ".ballerina-sheep-container",
      ".js-circle",
      ".js-bounce",
      ".js-cloud-container",
      ".js-fall",
    ...
    makeHotspotsAccessible: function () {
      var $hotspots = $(story.interactions.hotspotSelectors.join(", "));
      ...
      $hotspots.on("keydown.storyRuntimeA11y", function (e) {
  ...
  (story.effects = {
    ...
    bounce: function () {
      $(".js-bounce").on("click.storyRuntime", function (e) {
        if (!$(this).hasClass("js-running")) {
          ...
          story.effects.playAudio(story.config.sound_directory() + "/jump.mp3");
    ...
    stars: function (count) {
      ...
      story.runtime.startLoop(function (time) {
        ...
        stars.forEach(function (star) {
          var opacity =
            0.35 + (Math.sin(time * star.speed + star.phase) + 1) * 0.325;
    ...
    clouds: function (offscreenStart) {
      if (
        story.effects.reducedMotion() ||
        $(".js-cloud-container").length >= 10
      ) {
        return;
      }
      ...
      ((container = $(
        '<div class="cloud-container js-cloud-container js-glow" style="top:' +
          cloud_top +
          "%; left:" +
          cloud_start +
          "%; width:" +
      ...
      tl.add(
        TweenMax.to(container, story.effects.random_number(30, 100), {
          x: 1000,
          ease: Linear.easeNone,
        }),
      );
    ...
    random_number: function (numLow, numHigh) {
      ...
      var numRand =
        Math.floor(Math.random() * adjustedHigh) + parseFloat(numLow);
      return numRand;
    },
    ...
    fireworks: function () {
      ...
      for (i = 0; i < explosionQuantity; i++) {
        ...
        container.style.cssText =
          "position:absolute; left:0; top:0; overflow:visible; z-index:4; pointer-events:none;";
      ...
      function createExplosion(container) {
        ...
        //nothing to clone if the page has no .svg firework templates - bail out rather than throwing
        if (!svgArray.length) {
          return tl;
        }
        ...
        for (i = 0; i < fireworkQuantity; i++) {
          ...
          firework.append(
            svgArray[Math.floor(Math.random() * svgArray.length) + 0].cloneNode(
              true,
            ),
          );
          ...
          firework.classList.add(
            colourArray[Math.floor(Math.random() * colourArray.length) + 0],
          );
          firework.classList.add(
            sizeArray[Math.floor(Math.random() * sizeArray.length) + 0],
          );
    ...
    wind: function () {
      ...
      var element = $('<img src="./assets/img/common/wind1.png" />'),
        container = $(
          '<div class="wind" style="top:20%; left:20%; z-index: 150"></div>',
        );
      ...
      tl.add(
        TweenMax.to(element, story.effects.random_number(1, 3), {
          yPercent: story.effects.random_number(2000, 5000),
          repeat: 20,
          yoyo: true,
          ease: Sine.easeInOut,
        }),
      );
      TweenMax.to(container, story.effects.random_number(2, 3), {
        xPercent: "+=" + story.effects.random_number(8000, 10000),
        ease: Sine.easeInOut,
      });
    },
    blow: function () {
    ...
    splash: function () {
      ...
      for (i = 0; i < qty; i++) {
        ...
        dots.to(
          dot,
          duration,
          { physics2D: { velocity: velocity, angle: angle, gravity: gravity } },
          delay,
        );
      }
      dots.pause();
    ...
    randomMovementAndRepeat: function (element, range) {
      var xPercent = story.effects.random_number(
          range.xPercentMin,
          range.xPercentMax,
        ),
        yPercent = story.effects.random_number(
          range.yPercentMin,
          range.yPercentMax,
        ),
        movementTime = story.effects.random_number(
          range.timeMin,
          range.timeMax,
        );
      ...
      if (!!(movementTime % 2)) {
        ...
        tl.append(
          TweenMax.to(element, movementTime, {
            xPercent: xPercent,
            yPercent: yPercent,
            delay: 0.5,
          }),
    ...
    sun: function () {
      $(".js-sun").on("click.storyRuntime", function (e) {
        if (!$(this).hasClass("js-running")) {
          ...
          TweenMax.to($(".js-darken-layer"), 0.5, {
            autoAlpha: 0.9,
            repeatDelay: 0.5,
            repeat: 1,
            yoyo: true,
  ...
  (story.init = function () {
    ...
    var pageNo = window.location.pathname
      .split(/[\\\/]/)
      .pop()
      .replace(".html", "");
...
function triggerTouchClick(event) {
  if (
    (event.pointerType &&
...
document.addEventListener("pointerup", triggerTouchClick, { capture: true });
document.addEventListener(
  "touchend",
  function (event) {
    if (typeof PointerEvent === "undefined") {
      triggerTouchClick(event);
    }
  },
  { capture: true },
);
  ...
  function (event) {
    if (
      ...
      (event.target === lastTouchTarget ||
        lastTouchTarget.contains(event.target))
    ) {
...
//mobile browsers restore pages from bfcache on back/forward without re-running init, leaving every .storyRuntime handler stripped by unmount() above
